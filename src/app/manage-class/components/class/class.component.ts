...
export class ClassComponent implements OnInit {
  ...
  delete() {
    const messageDialogRef = this.dialog.open(MessageBoxComponent , {
      data : { title : this.message.box.title , message : this.message.box.message , confirm : this.message.box.confirm }
     });

     messageDialogRef.afterClosed().subscribe(
       (res) => {
         if (res) {
          this.subDelelte = this.teamService.deleteTeam(this.team.id).subscribe(
            (result: Result) => {
              console.log(result);
            }
          );
